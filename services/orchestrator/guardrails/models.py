...
class GuardrailConfig(BaseModel):
    ...
    enable_harmful_content_filter: bool = Field(
        default=True,
        description="Enable harmful content filtering",
    )
    ...
    block_on_pii: bool = Field(
        default=False,
        description="Block request on PII detection vs just warn",
    )
    block_on_injection: bool = Field(
        default=True,
        description="Block request on injection detection",
    )
    block_on_harmful_content: bool = Field(
        default=True,
        description="Block response on harmful content",
    )

    # Severity thresholds
    pii_severity: Literal["low", "medium", "high", "critical"] = Field(
        default="medium",
        description="Default severity for PII violations",
    )
    injection_severity: Literal["low", "medium", "high", "critical"] = Field(
        default="critical",
        description="Default severity for injection violations",
    )
    harmful_content_severity: Literal["low", "medium", "high", "critical"] = Field(
        default="high",
        description="Default severity for harmful content violations",
    )
    length_severity: Literal["low", "medium", "high", "critical"] = Field(
        default="low",
        description="Default severity for length violations",
    )
