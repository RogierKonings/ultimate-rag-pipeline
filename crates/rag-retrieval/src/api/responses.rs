...
pub struct DebugInfo {
    ...
    /// Query expansion variations used for semantic embedding (excluding original).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub expanded_queries: Vec<String>,

    /// Number of text inputs used to build the semantic embedding.
    #[serde(default)]
    pub embedding_input_count: usize,

    /// Whether `HyDE` was successfully used for this request.
    #[serde(default)]
    pub hyde_used: bool,

    /// Number of hypothetical documents generated by `HyDE`.
    #[serde(default)]
    pub hyde_generated_docs: usize,

    /// Time spent generating `HyDE` documents (ms).
    #[serde(default)]
    pub hyde_latency_ms: f64,

    /// Runtime fallback applied when a hybrid component failed.
    #[serde(skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn test_retrieve_response_creation() {
        ...
        let response =
            RetrieveResponse::new(results, "test query".into(), SearchMode::Hybrid, metrics);
